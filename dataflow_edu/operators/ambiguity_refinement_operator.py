# -*- coding: utf-8 -*-
"""
3.2 Ambiguity Refinement Operator - 对中质量（2–3 分）题目优化二义性

从 3.1 清洗结果中筛选 2–3 分题，通过 LLM 优化题干与答案，并与 4-5 分题合并输出。
"""
...
from dataflow_edu.ambiguity_refinement.core import (
    _print_ambiguity_distribution,
    _scan_refinement_candidates,
    run_ambiguity_refinement,
)
from dataflow_edu.config.schema import AmbiguityRefinementConfig, EduConfig
...
def _check_refined_status(folder_name: str, output_dir: str) -> bool:
    """检查该教材是否已完成二义性精修。"""
    path = os.path.join(output_dir, f"{folder_name}_ambiguity_refined.json")
    return os.path.isfile(path)


def _display_refinement_table(candidates: list, input_dir: str, output_dir: str):
    print(f"\n{'=' * 60}")
    print("3.2 Ambiguity Refinement Operator - 可选教材")
    print(f"{'=' * 60}")
...
class AmbiguityRefinementOperator(OperatorABC):
    """
    3.2 Ambiguity Refinement Operator：对中质量（2–3 分）样本优化二义性。
    从 3.1 清洗结果筛选 2–3 分题，LLM 优化题干与答案，与 4-5 分题合并输出。
    """
    ...
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            return (
                "3.2 Ambiguity Refinement Operator：对中质量（3 分）样本优化二义性。"
                "从 3.1 清洗结果筛选 2–3 分题，LLM 优化题干与答案，与 4-5 分题合并输出。"
            )
        return "3.2 Ambiguity Refinement Operator: refine medium-quality (score 2-3) samples."
    ...
    def run(
        ...
        """
        执行二义性精修：扫描 3_1_ambiguity_cleaned -> 交互选择 -> API 配置 -> 优化 -> 保存。
        """
        ...
        ref_cfg = config.operators.get("ambiguity_refinement")
        if isinstance(ref_cfg, dict):
            ...
            ref_cfg = AmbiguityRefinementConfig(
                ...
                target_scores=ts,
                threshold_discard=int(ref_cfg.get("threshold_discard", 2)),
            )
        elif not isinstance(ref_cfg, AmbiguityRefinementConfig):
            ref_cfg = AmbiguityRefinementConfig(
                input_dir=input_dir,
        ...
        if not candidates:
            self.logger.warning("没有找到可精修的教材（需先完成 3.1 Ambiguity Cleaning）")
            print("未找到教材，请先运行 3.1 Ambiguity Cleaning 完成 3_1_ambiguity_cleaned。")
            return False, None
        ...
        input_path = os.path.join(input_dir_resolved, f"{selected}_ambiguity_cleaned.json")
        ...
        ok, refined_path = run_ambiguity_refinement(
        ...
        if ok and refined_path and os.path.isfile(refined_path):
            try:
                ...
                pass
            print(f"\n3.2 Ambiguity Refinement 完成: {refined_path}")
